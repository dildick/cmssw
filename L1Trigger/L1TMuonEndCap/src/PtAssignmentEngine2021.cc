...
const PtAssignmentEngineAux2021& PtAssignmentEngine2021::aux() const {
  static const PtAssignmentEngineAux2021
      instance;  // KK: arguable design solution, but const qualifier makes it thread-safe anyway
  return instance;
}

float PtAssignmentEngine2021::scale_pt(const float pt, const int mode) const {
...
float PtAssignmentEngine2021::unscale_pt(const float pt, const int mode) const {
  emtf_assert(ptLUTVersion_ >= 6);
...
PtAssignmentEngine::address_t PtAssignmentEngine2021::calculate_address(const EMTFTrack& track) const {
  address_t address = 0;
  ...
  return address;
}  // End function: PtAssignmentEngine2021::calculate_address()

// Calculate XML pT from address
float PtAssignmentEngine2021::calculate_pt_xml(const address_t& address) const {
...
}  // End function: float PtAssignmentEngine2021::calculate_pt_xml(const address_t& address)

// Calculate XML pT directly from track quantities, without forming an address
float PtAssignmentEngine2021::calculate_pt_xml(const EMTFTrack& track) const {
  float pt_xml = 0.;
  ...
  // END: Identical (almost) to BDT training code in EMTFPtAssign2021/PtRegression_Apr_2021.C
...
}  // End function: float PtAssignmentEngine2021::calculate_pt_xml(const EMTFTrack& track)
