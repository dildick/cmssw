...
L1TMuonShowerProducer::L1TMuonShowerProducer(const edm::ParameterSet& iConfig)
    : showerInputTag_(iConfig.getParameter<edm::InputTag>("showerInput")),
      ...
      enableNominalShowers_(iConfig.getParameter<bool>("enableNominalShowers")),
      enableTwoLooseShowers_(iConfig.getParameter<bool>("enableTwoLooseShowers")) {
...
void L1TMuonShowerProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t i = 0; i < emtfShowers->size(0); ++i) {
    ...
    if (shower.isValid()) {
      // nominal
      if (shower.isOneNominalInTime() and enableNominalShowers_)
        isOneNominalInTime = true;
      if (shower.isOneNominalOutOfTime() and enableNominalShowers_)
        isOneNominalOutOfTime = true;
      // two loose
      if (shower.isTwoLooseInTime() and enableTwoLooseShowers_)
        isTwoLooseInTime = true;
      if (shower.isTwoLooseOutOfTime() and enableTwoLooseShowers_)
        isTwoLooseOutOfTime = true;
    }
  }

  // Check for at least one shower
  const bool accept(isOneNominalInTime or isOneNominalOutOfTime or isTwoLooseInTime or isTwoLooseOutOfTime);
  if (accept) {
...
void L1TMuonShowerProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("enableNominalShowers", true);
