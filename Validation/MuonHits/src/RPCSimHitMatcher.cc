...
void RPCSimHitMatcher::match(const SimTrack& track, const SimVertex& vertex)
{
  ...
  if (hasGeometry_) {
    ...
    if (verbose_) {
      cout<<"nSimHits "<<simHits_.size()<<" nTrackIds "<<track_ids_.size()<<endl;
      cout << "detids RPC " << detIds().size() << endl;

      const auto& ch_ids = chamberIds();
      for (const auto& id: ch_ids) {
        const auto& simhits = hitsInChamber(id);
...
void
RPCSimHitMatcher::matchSimHitsToSimTrack(std::vector<unsigned int> track_ids_, const edm::PSimHitContainer& hits)
{
...
RPCSimHitMatcher::detIds(int type) const
{
  ...
  for (const auto& p: detid_to_hits_)
  {
    ...
    {
      RPCDetId detId(id);
      if (MuonHitHelper::toRPCType(detId.region(), detId.station(), detId.ring()) != type) continue;
    }
    result.insert(id);
  }
  return result;
...
std::set<unsigned int>
RPCSimHitMatcher::chamberIds(int type) const
{
  std::set<unsigned int> result;
...
int
RPCSimHitMatcher::nStations() const
{
  return (hitStation(1) + hitStation(2) + hitStation(3) + hitStation(4));
}

float
RPCSimHitMatcher::simHitsMeanStrip(const edm::PSimHitContainer& sim_hits) const
{
...
std::set<int>
RPCSimHitMatcher::hitStripsInDetId(unsigned int detid, int margin_n_strips) const
{
  set<int> result;
  RPCDetId id(detid);
