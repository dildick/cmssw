...
void DTSimHitMatcher::match(const SimTrack& track, const SimVertex& vertex)
{
  ...
  if (hasGeometry_) {
    ...
    if (verbose_) {
      ...
      cout<<"detids DT " << detIds().size()<<endl;
...
void
DTSimHitMatcher::matchSimHitsToSimTrack(std::vector<unsigned int> track_ids, const edm::PSimHitContainer& dt_hits)
{
...
DTSimHitMatcher::detIds(int dt_type) const
{
...
std::set<unsigned int>
DTSimHitMatcher::chamberIds(int dt_type) const
{
  std::set<unsigned int> result;
...
std::set<unsigned int>
DTSimHitMatcher::layerIds() const
{
  std::set<unsigned int> result;
...
std::set<unsigned int>
DTSimHitMatcher::superlayerIds() const
{
  std::set<unsigned int> result;
...
DTSimHitMatcher::hitsInLayer(unsigned int detid) const
{
  if (!MuonHitHelper::isDT(detid)) return no_hits_;

  const DTWireId id(detid);
  if (layer_to_hits_.find(id.layerId().rawId()) == layer_to_hits_.end()) return no_hits_;
  return layer_to_hits_.at(id.layerId().rawId());
}

const edm::PSimHitContainer&
DTSimHitMatcher::hitsInSuperLayer(unsigned int detid) const
{
  if (!MuonHitHelper::isDT(detid)) return no_hits_;

  const DTWireId id(detid);
  if (superlayer_to_hits_.find(id.superlayerId().rawId()) == superlayer_to_hits_.end()) return no_hits_;
  return superlayer_to_hits_.at(id.superlayerId().rawId());
}

const edm::PSimHitContainer&
DTSimHitMatcher::hitsInChamber(unsigned int detid) const
{
  if (!MuonHitHelper::isDT(detid)) return no_hits_;

  const DTWireId id(detid);
  if (chamber_to_hits_.find(id.chamberId().rawId()) == chamber_to_hits_.end()) return no_hits_;
  return chamber_to_hits_.at(id.chamberId().rawId());
}

bool
DTSimHitMatcher::hitStation(int st, int nsuperlayers, int nlayers) const
{
  int nst=0;
...
int
DTSimHitMatcher::nStations(int nsuperlayers, int nlayers) const
{
...
int
DTSimHitMatcher::nCellsWithHitsInLayer(unsigned int detid) const
{
...
int
DTSimHitMatcher::nLayersWithHitsInSuperLayer(unsigned int detid) const
{
  set<int> layers_with_hits;
...
int
DTSimHitMatcher::nLayersWithHitsInChamber(unsigned int detid) const
{
  int nLayers = 0;
...
float
DTSimHitMatcher::simHitsMeanWire(const edm::PSimHitContainer& sim_hits) const
{
...
std::set<unsigned int>
DTSimHitMatcher::hitWiresInDTLayerId(unsigned int detid, int margin_n_wires) const
{
    ...
    for (int wn = 0; wn <= max_nwires; ++wn) {
      ...
      for (const auto& h: MuonSimHitMatcher::hitsInDetId(wid.rawId())) {
        ...
        for (int ss = smin; ss <= smax; ++ss) {
          DTWireId widd(id, ss);
          if (verbose_) cout << "\tadding DTWireId to collection "<< widd << endl;
...
std::set<unsigned int>
DTSimHitMatcher::hitWiresInDTSuperLayerId(unsigned int detid, int margin_n_wires) const
{
  set<unsigned int> result;
...
std::set<unsigned int>
DTSimHitMatcher::hitWiresInDTChamberId(unsigned int detid, int margin_n_wires) const
{
  set<unsigned int> result;
  const auto& superLayers(dynamic_cast<const DTGeometry*>(geometry_)->chamber(DTChamberId(detid))->superLayers());
  for (const auto& sl: superLayers) {
...
void
DTSimHitMatcher::dtChamberIdsToString(const std::set<unsigned int>& set) const
{
...
std::set<unsigned int>
DTSimHitMatcher::chamberIdsStation(int station) const
{
  set<unsigned int> result;
  switch(station){
  case 1: {
